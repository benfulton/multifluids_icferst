...
contains
  ...
  function expand_mesh_halo(mesh) result (new_mesh)
    ...
    do proc=1, nprocs
      ...
      do i=1, key_count(elements_to_send)
        ...
        ! ewrite(1,*) "  Sending info for element with uid", ele_uid, "with nodes", nodes
      ...
      !ewrite(1,*) "  Sending to proc", proc, "number of elements:", size(send_buffer(proc)%ptr)/ele_info_size
      !ewrite(1,'(A,1X,*(Z0,1X))') "  First 10 bytes of send_buffer:", send_buffer(proc)%ptr(1:min(10, size(send_buffer(proc)%ptr)))
      call MPI_ISend(send_buffer(proc)%ptr, send_count-1, getpinteger(), &
        proc-1, tag,  comm, send_request(proc), ierr)
      if(ierr /= MPI_SUCCESS) then
        ewrite(1, *) "MPI_ISend failed with ierr = ", ierr
        FLAbort("MPI_ISend failed")
      end if
    ...
    ! ewrite(1,*) "Receiving data for numbering new elements"
    do proc=1, nprocs
      if (.not. ((halo_send_count(new_halo, proc)==0) .eqv. (halo_receive_count(new_halo, proc)==0))) then
        FLAbort("halo_send_count and halo_receive_count mismatch")
      end if

      if (halo_receive_count(new_halo, proc)==0) cycle
      call mpi_probe(proc-1, tag, comm, status, ierr)
      if(ierr /= MPI_SUCCESS) then
        ewrite(1, *) "MPI_Probe failed with ierr = ", ierr
        FLAbort("MPI_Probe failed")
      end if


      call mpi_get_count(status, getpinteger(), recv_size, ierr)
      if(ierr /= MPI_SUCCESS) then
        ewrite(1, *) "MPI_Get_count failed with ierr = ", ierr
        FLAbort("MPI_Get_count failed")
      end if
      ...
      if(ierr /= MPI_SUCCESS) then
        ewrite(1, *) "MPI_Recv failed with ierr = ", ierr
        FLAbort("MPI_Recv failed")
      end if

      !ewrite(1,*) "  Received from proc", proc, "number of elements:", recv_size/ele_info_size
      !ewrite(1,'(A,1X,*(Z0,1X))') "  First 10 bytes of recv_buffer:", recv_buffer(proc)%ptr(1:min(10, size(recv_buffer(proc)%ptr)))
      
      ele_loop: do i=1, recv_size/ele_info_size
        ele_uid = recv_buffer(proc)%ptr((i-1)*ele_info_size+1)
        !ewrite(1,*) "  Received info for element with uid", ele_uid
        if (.not. has_key(ueid_to_gid, ele_uid)) then
          ...
          !ewrite(1,*) "  Accepting element", new_element_count, "with uid", ele_uid, "to", nodes
          !ewrite(1,*) "  with global nodes", fetch(uid_to_gid, nodes)
    ...
    if(ierr /= MPI_SUCCESS) then
        ewrite(1, *) "MPI_WaitAll failed with ierr = ", ierr
        FLAbort("MPI_WaitAll failed")
    end if
    ...
    do proc=1, nprocs
      !ewrite(1,*) "Processing recv buffer from proc", proc, "with size", size(recv_buffer(proc)%ptr)
      if (halo_receive_count(new_halo, proc)==0) cycle
      ...
      do i=1, recv_size/ele_info_size
        ...
        if (has_key(ueid_to_gid, ele_uid)) then
          ...
          nodes => recv_buffer(proc)%ptr((i-1)*ele_info_size+2:(i-1)*ele_info_size+1+nloc)
          !ewrite(1,*) "Setting nodes for existing element", ele, "with uid", ele_uid, "to", nodes
  ...
  function expand_halo(mesh, halo, name) result (new_halo)
    ...
    do proc=1, nprocs
      ...
      if(ierr /= MPI_SUCCESS) then
        ewrite(1, *) "MPI_IRecv failed with ierr = ", ierr
        FLAbort("MPI_IRecv failed")
      end if
    end do
    ...
    do proc=1, nprocs
      ...
      if(ierr /= MPI_SUCCESS) then
        ewrite(1, *) "MPI_Wait failed with ierr = ", ierr
        FLAbort("MPI_Wait failed")
      end if
      recv_count = 0
      do i=1, halo_receive_count(halo, proc)
        node = halo_receive(halo, proc, i)
    ...
    if (node_count(mesh) /= key_count(uid_to_gid)) then
      FLAbort("Old halo has duplicate nodes - aborting")
    end if
